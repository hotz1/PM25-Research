...
# The dataset contains Level 2 Aerosol parameters, with dates ranging from March 1, 2000 until February 28, 2022 (as of November 28, 2022)
...
for(year in 2000:2022){
  ...
  download.misr.urls(start = paste0(year, '-01-01'), end = paste0(year, '-12-31'),
                     url.folder = 'Data/MISR_Large/MISR_urls/temp_files')
  ...
  misr.urls = paste0('./Data/MISR_Large/MISR_urls/temp_files/', list.files('./Data/MISR_Large/MISR_urls/temp_files'))
  ...
  saveRDS(ncdf.urls, paste0('./Data/MISR_Large/MISR_urls/ncdf_urls_', year, '.rds'))
  
  # Find the file paths of the downloaded files and remove them from the temporary file folder
  misr.filepaths = list.files('./Data/MISR_Large/MISR_urls/temp_files', pattern = ".html", full.names = TRUE)
...
# 
# ncdf.folder = 'Data/MISR_Large/NetCDF_files'
# ncdf.folder = paste0(getwd(), '/', ncdf.folder, '/')
# 
# for(year in 2000:2022){
#   # Read vector from RDS object to resume downloading
#   ncdf.urls = readRDS(paste0('./Data/MISR_Large/MISR_urls/ncdf_urls_', year, '.rds'))
#   
