...
PM25_AOD_XGB_RMSE <- rep(NA, times = nrow(XGB_hyperparams))
PM25_AOD_XGB_R2 <- rep(NA, times = nrow(XGB_hyperparams))
...
# Empty (filled later) list of variable importances for each xgboost model
PM25_AOD_XGB_varImp <- vector(mode = "list", length = nrow(XGB_hyperparams))

# Set up the trainControl used in the caret package to fit the model
train_control <- trainControl(method = "none", search = "grid")

for(i in 1:nrow(XGB_hyperparams)){
  # Fit XGB model with the given hyperparameters
  PM25_AOD_XGB <- caret::train(PM25 ~ ., data = PM25_AOD.train, method = 'xgbTree', 
                               trControl = train_control, tuneGrid = XGB_hyperparams[i,])
  
  PM25_AOD_XGB_varImp[i] <- varImp(PM25_AOD_XGB, scale = F)
  
  # Compute predicted values in the validation set based on the fitted model
  PM25_AOD_XGB_pred <- predict(PM25_AOD_XGB, newdata = PM25_AOD.validation)
  ...
  # Compute predicted values in the test set based on the fitted model
  PM25_AOD_XGB_pred_test <- predict(PM25_AOD_XGB, newdata = PM25_AOD.test)
  
  # Compute test set RMSE and R2 of the fitted XGB model
  PM25_AOD_XGB_test_RMSE[i] <- caret::RMSE(PM25_AOD_XGB_pred_test, PM25_AOD.test$PM25)
  PM25_AOD_XGB_test_R2[i] <- caret::R2(PM25_AOD_XGB_pred_test, PM25_AOD.test$PM25)
}

# Create and display table of all XGBoost models
PM25_AOD_XGB_table <- cbind(XGB_hyperparams, RMSE = PM25_AOD_XGB_RMSE, R2 = PM25_AOD_XGB_R2)
PM25_AOD_XGB_table %>%
...
PM25_AOD_XGB_varImp[[which.min(PM25_AOD_XGB_RMSE)]] %>%
  knitr::kable(caption = "Variable Importance of the XGB model with the lowest RMSE")

# Variable Importance matrix of the XGB model with the best RMSE
PM25_AOD_XGB_varImp[[which.max(PM25_AOD_XGB_R2)]] %>%
  knitr::kable(caption = "Variable Importance of the XGB model with the highest R2")
```
