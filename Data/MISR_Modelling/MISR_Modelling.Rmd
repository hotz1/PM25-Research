...
# Empty (filled later) vector of xgboost models
pm25_aod_xgb_models <- rep(NA, times = nrow(pm25_aod_xgb_params))
...
for(i in 1:nrow(pm25_aod_xgb_params)){
  ...
  pm25_aod_xgb_models[i] <- xgboost(data = as.matrix(PM25_AOD.train)[,2:15], 
                                    label = as.matrix(PM25_AOD.train)[,1],
  ...
  # Predict values for the validation set with the xgboost model
  PM25_AOD_xgb_pred <- predict(pm25_aod_xgb_models[[i]], newdata = as.matrix(PM25_AOD.validation)[,2:15])
  
  # Compute RMSE and R2 of data on the validation dataset
...
# Create and display table of all 72 XGBoost models
...
# Select models with best RMSE and best R2
pm25_aod_xgb_best_RMSE <- pm25_aod_xgb_models[[which.min(pm25_aod_xgb_RMSE)]]
pm25_aod_xgb_best_R2 <- pm25_aod_xgb_models[[which.max(pm25_aod_xgb_R2)]]
```
