...
XGB_hyperparams <- XGB_hyperparams[1:5] #TEST CODE REMOVE LATER
...
for(i in 1:nrow(XGB_hyperparams)){
  ...
  PM25_AOD_XGB_importance_plots[[i]] <- xgb.ggplot.importance(PM25_AOD_XGB_importance, 
                                                              rel_to_first = FALSE, n_clusters = 2)
  ...
  predicted_vs_real_plots[[i]] <- plot_pred_vs_obs(observed = PM25_AOD.test$PM25, predicted = XGB_test_predicted,
                                                   ...
                                                   subtitle = "Values were predicted using an XGBoost model")
}
...
PM25_AOD_XGB_table %>% 
  ...
  labs(x = "Validation RMSE", y = "Validation R2", title = "RMSE and R2 values of XGBoost models on the validation dataset",
       subtitle = paste("Based on", nrow(XGB_hyperparams), "fitted XGBoost models")) +
  theme_bw()

if(which.min(PM25_AOD_XGB_RMSE) == which.max(PM25_AOD_XGB_R2)){
  # Variable Importance matrix of the XGB model with the best RMSE
  PM25_AOD_XGB_varImp[[which.min(PM25_AOD_XGB_RMSE)]] %>%
    knitr::kable(caption = "Variable Importance of the best XGBoost model (lowest RMSE and highest R2)")
} else {
  # Variable Importance matrix of the XGB model with the best RMSE
  PM25_AOD_XGB_varImp[[which.min(PM25_AOD_XGB_RMSE)]] %>%
    knitr::kable(caption = "Variable Importance of the XGBoost model with the lowest RMSE")
  
  # Variable Importance matrix of the XGB model with the best RMSE
  PM25_AOD_XGB_varImp[[which.max(PM25_AOD_XGB_R2)]] %>%
    knitr::kable(caption = "Variable Importance of the XGBoost model with the highest R2")
}
```
